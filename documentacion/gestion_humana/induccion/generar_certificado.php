<?php
// documentacion/gestion_humana/induccion/generar_certificado.php
...
function carpetaCompletadaCert($usuario_id, $carpeta_id) {
    global $pdo;
...
function induccionCompletadaCert($usuario_id) {
    ...
    foreach ($areas as $area_id) {
        ...
        foreach ($carpetas as $carpeta_id) {
            if (!carpetaCompletadaCert($usuario_id, $carpeta_id)) {
                return false;
            }
        }
    }
...
function obtenerProgresoCert($usuario_id) {
    ...
    foreach ($areas as $area) {
        ...
        foreach ($carpetas as $carpeta) {
            $completada = carpetaCompletadaCert($usuario_id, $carpeta['id']);
            $carpetas_detalle[] = [
...
// ============================================
// VERIFICAR QUE LA INDUCCIÓN ESTÉ COMPLETADA
// ============================================

if (!induccionCompletadaCert($usuario_id)) {
    header('Location: index.php?error=❌ Debes completar la inducción al 100% para generar el certificado.');
    exit();
}

$progreso = obtenerProgresoCert($usuario_id);
...
$pdf->Output('Certificado_Induccion_' . $usuario['usuario'] . '.pdf', 'D');
