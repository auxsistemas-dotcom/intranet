<?php
// admin/capacitaciones/editar_area.php - Editar área
...
elseif ($rol_usuario == 2) {
    ...
    if (!$permiso || $permiso['tiene_permiso'] == 0) {
        $_SESSION['error'] = "❌ No tienes permiso para editar áreas";
...
// ✅ Obtener datos del área (SOLO tipo 'area')
$stmt = $pdo->prepare("SELECT * FROM categorias_capacitaciones WHERE id = ? AND tipo = 'area'");
$stmt->execute([$id]);
$item = $stmt->fetch(PDO::FETCH_ASSOC);

if (!$item) {
    header("Location: index.php?error=" . urlencode("❌ Área no encontrada"));
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nombre)) {
        ...
        try {
            // ✅ Actualizar área (SOLO nombre y descripción)
            $stmt = $pdo->prepare("UPDATE categorias_capacitaciones SET nombre = ?, descripcion = ? WHERE id = ?");
        ...
        <div class="card">
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <div class="info-text">Describe brevemente el propósito de esta área</div>
                </div>
