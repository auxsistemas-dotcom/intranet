<?php
// admin/politicas_armotor/index.php - Gestionar Políticas ARMOTOR
...
if (isset($_GET['eliminar']) && is_numeric($_GET['eliminar'])) {
    ...
    $stmt = $pdo->prepare("SELECT archivo_url FROM politicas WHERE id = ?");
    $stmt->execute([$id]);
    ...
    $stmt = $pdo->prepare("DELETE FROM politicas WHERE id = ?");
    $stmt->execute([$id]);
    
    $_SESSION['mensaje'] = "✅ Política eliminada correctamente";
    header("Location: index.php");
    exit();
}
...
if (isset($_GET['cambiar_estado']) && is_numeric($_GET['cambiar_estado'])) {
    ...
    $stmt = $pdo->prepare("UPDATE politicas SET activo = NOT activo WHERE id = ?");
    $stmt->execute([$id]);
...
$stmt = $pdo->query("SELECT * FROM politicas ORDER BY orden ASC, titulo ASC");
...
<head>
    ...
    <title>Gestionar Políticas | INTRANET</title>
...
<body>
    ...
    <div class="container">
        ...
        <!-- ===== FORMULARIO AGREGAR POLÍTICA ===== -->
        <div class="card">
            <h2><i class="fas fa-plus-circle"></i> Agregar nueva política</h2>
            <form action="guardar.php" method="POST" enctype="multipart/form-data">
                <div class="form-group">
                    <label><i class="fas fa-heading"></i> Título</label>
                    <input type="text" name="titulo" required placeholder="Ej: Política de Seguridad 2025">
                </div>
                <div class="form-group">
        ...
        <div class="card">
            ...
            <?php if (empty($politicas)): ?>
                <div class="empty-state">
                    ...
                    <h3>No hay políticas registradas</h3>
                    <p>Agrega la primera política usando el formulario superior.</p>
                </div>
            <?php else: ?>
                            ...
                            <?php foreach ($politicas as $doc): ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <a href="?eliminar=<?php echo $doc['id']; ?>" class="btn-accion btn-eliminar" title="Eliminar" onclick="return confirm('¿Eliminar esta política permanentemente?')">
