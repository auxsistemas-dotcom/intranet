<?php
// admin/gestionar_pagina/slider/crear.php - Crear nuevo slide
...
if ($rol_usuario != 1 && $rol_usuario != 2) {
    $_SESSION['error'] = "❌ No tienes permiso para crear slides";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($titulo)) {
        ...
        try {
            $stmt = $pdo->prepare("
                INSERT INTO slider (titulo, descripcion, imagen_url, tipo, enlace, orden, activo, creado_por) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $ruta_imagen = 'uploads/slider/' . $imagen_nombre;
            $stmt->execute([$titulo, $descripcion, $ruta_imagen, $tipo, $enlace, $orden, $activo, $usuario_id]);
            
            $_SESSION['mensaje'] = "✅ Slide creado correctamente";
            header("Location: index.php");
            exit();
            
        } catch (PDOException $e) {
            $error = "❌ Error al crear el slide: " . $e->getMessage();
        }
    }
}

// Obtener el último orden para sugerir
$stmt = $pdo->query("SELECT MAX(orden) as max_orden FROM slider");
$max_orden = $stmt->fetch(PDO::FETCH_ASSOC)['max_orden'] ?? 0;
$sugerir_orden = $max_orden + 1;
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <title>Crear Slide | INTRANET</title>
...
<body>
    ...
    <div class="container">
        ...
        <div class="card">
            <h2><i class="fas fa-plus-circle"></i> Nuevo Slide</h2>

            <form method="POST" action="" enctype="multipart/form-data">
                <div class="form-group">
                    <label>Título *</label>
                    <input type="text" name="titulo" placeholder="Ej: Innovación que Mueve tu Negocio" value="<?php echo htmlspecialchars($titulo ?? ''); ?>" required>
                </div>

                <div class="form-group">
                    <label>Descripción</label>
                    <textarea name="descripcion" placeholder="Escribe la descripción del slide..."><?php echo htmlspecialchars($descripcion ?? ''); ?></textarea>
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        <label>Tipo</label>
                        <select name="tipo">
                            <option value="noticia" <?php echo ($tipo ?? 'noticia') == 'noticia' ? 'selected' : ''; ?>>Noticia</option>
                            <option value="promocion" <?php echo ($tipo ?? '') == 'promocion' ? 'selected' : ''; ?>>Promoción</option>
                            <option value="aviso" <?php echo ($tipo ?? '') == 'aviso' ? 'selected' : ''; ?>>Aviso</option>
                            <option value="evento" <?php echo ($tipo ?? '') == 'evento' ? 'selected' : ''; ?>>Evento</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label>Orden</label>
                        <input type="number" name="orden" value="<?php echo $sugerir_orden; ?>" min="0">
                    </div>
                </div>

                <div class="form-group">
                    <label>Enlace (opcional)</label>
                    <input type="url" name="enlace" placeholder="https://ejemplo.com" value="<?php echo htmlspecialchars($enlace ?? ''); ?>">
                    <div class="info-text">URL a la que redirigirá al hacer clic en el slide</div>
                </div>

                <div class="form-group checkbox-group">
                    <input type="checkbox" name="activo" <?php echo ($activo ?? 1) ? 'checked' : ''; ?>>
